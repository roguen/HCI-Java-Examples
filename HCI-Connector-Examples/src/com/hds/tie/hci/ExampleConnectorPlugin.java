...
import com.hds.ensemble.sdk.exception.PluginOperationNotFoundException;
 ...
 * crawled.
 */
public class ExampleConnectorPlugin implements ConnectorPlugin, DocumentActionProcessor {

    private static final String PLUGIN_NAME = "com.hds.ensemble.sdk.examples.connector.exampleConnector";
    ...
    public ExampleConnectorPlugin() {
        this.config = null;
        this.callback = null;
    }

    // Constructor for configured plugin instances to be used in workflows
    private ExampleConnectorPlugin(PluginConfig config, PluginCallback callback)
    ...
    @Override
    public ExampleConnectorPlugin build(PluginConfig config, PluginCallback callback)
            throws ConfigurationException {
        validateConfig(config);

        // This method is used as a factory to create a configured instance of this connector
        return new ExampleConnectorPlugin(config, callback);
    }
